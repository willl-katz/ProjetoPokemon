...
const btnCloseModal = document.querySelector('.js-close-modal-datails-pokemon');
const areaOutersAbilits = document.querySelector('.outras-habilidades');

if (btnCloseModal) {
    btnCloseModal.addEventListener('click', closeDetailsPokemon);
}

const btnDropdownSelect = document.querySelector('.js-open-select-cumtom');
...
function createCardPokemon(code, type, name, imagePok) {
    ...
    card.classList = `card-pokemon js-open-datails-pokemon ${type}`;
    ...
    .then(response => {
        ...
        results.forEach(pokemon => {
            ...
            .then(response => {
                ...
                const cardPokemon = document.querySelectorAll('.js-open-datails-pokemon');
...
const btnOutersAbilits = document.getElementById('js-btn-outers-abilits');
const btnCloseOutersAbilits = document.getElementById('js-btn-outers-abilits-close');
...
function openDetailsPokemon() {
    ...
    let codeInfoPokemon = this.querySelector('.info span').textContent;

    const imgPokemonModal = document.getElementById('js-image-pokemon-modal');
    const modalDatails = document.getElementById('js-modal-datails');
    const iconTypePokemonModal = document.getElementById('js-image-type-icon');
    ...
    .then(response => {
        ...
        let infoPokemon = {
            mainAbilities: data.abilities[0].ability.name,
            types: data.types,
            weight: data.weight,
            height: data.height,
        ...
        const statsDefence = document.getElementById('js-stats-defense');
        ...
        const storageOutersAbilits = document.getElementById('js-storage-outers-abilits');

        statsHp.style.width = `${infoPokemon.stats[0].base_stat}%`;
        statsAttack.style.width = `${infoPokemon.stats[1].base_stat}%`;
        statsDefence.style.width = `${infoPokemon.stats[2].base_stat}%`;
        ...
        function listingOpenOutersAbilits() {
            btnOutersAbilits.style.display = 'none';
            btnCloseOutersAbilits.style.display = 'flex';
            areaOutersAbilits.classList.add('active');
            let arrayOutersAbilits = infoPokemon.abilities;

            arrayOutersAbilits.forEach((abilits, index) => {
                let namesAbilits = abilits.ability.name;
                storageOutersAbilits.innerHTML = '';

                if(index > 0) {
                    let boxNameOutersAbilits = document.createElement('li');
                    storageOutersAbilits.appendChild(boxNameOutersAbilits);

                    let NameOutersAbilits = document.createElement('span');
                    NameOutersAbilits.textContent = `${namesAbilits}`;
                    boxNameOutersAbilits.appendChild(NameOutersAbilits);
                } else {
                    let NameDefaultOutersAbilits = document.createElement('span');
                    NameDefaultOutersAbilits.textContent = `Não possui mais habilidades...`;
                    NameDefaultOutersAbilits.classList = 'default-outers-abilits';
                    storageOutersAbilits.appendChild(NameDefaultOutersAbilits);
                }
            });
        }

        function listingCloseOutersAbilits() {
            storageOutersAbilits.innerHTML = '';
            btnOutersAbilits.style.display = 'flex';
            btnCloseOutersAbilits.style.display = 'none';
            areaOutersAbilits.classList.remove('active');
        }

        listingTypesPokemon()
        listingWeak()
        btnOutersAbilits.addEventListener('click', listingOpenOutersAbilits);
        btnCloseOutersAbilits.addEventListener('click', listingCloseOutersAbilits);
    })
}
...
function closeDetailsPokemon() {
    btnOutersAbilits.style.display = 'flex';
    btnCloseOutersAbilits.style.display = 'none';
    if(areaOutersAbilits.classList.contains('active')) {
        areaOutersAbilits.classList.remove('active');
    }
    ...
    if(valueInput != '') {
        ...
        .catch(error => {
            if(error.response) {
                ...
                Swal.fire({
                    title: 'Poxa!',
                    text: 'Não encontramos seu Pokemon',
                    imageUrl: '../assets/pokemon-triste.jpg',
                    imageWidth: 300,
